  ...
  return (
    <main className="section-pad">
      <div className="container-shell">
        <section className="rounded-[36px] bg-white p-6 shadow-[0_16px_36px_rgba(0,0,0,0.06)] sm:p-8">
          <SectionHeading
        ...
        <section className="mt-8 grid gap-4 lg:grid-cols-4">
          {deliveryZones.map((zone) => (
            <article key={zone.id} className="card-white rounded-[28px] p-5">
              ...
              <div className="mt-4 text-3xl font-black text-[#171717]">{zone.priceLabel}</div>
              <div className="mt-2 text-sm text-[#6D6D6D]">{zone.distance}</div>
              <p className="mt-4 text-sm leading-6 text-[#6D6D6D]">{zone.description}</p>
              <div className="mt-5 rounded-[18px] bg-[#F6F6F4] px-4 py-3 text-sm font-semibold text-[#171717]">{zone.eta}</div>
            </article>
          ))}
        </section>
        ...
        <section className="mt-8 grid gap-4 lg:grid-cols-[0.9fr_1.1fr]">
          ...
          <div className="grid gap-4">
            {[
              ...
              <div key={item} className="card-white rounded-[26px] p-5 text-sm leading-6 text-[#6D6D6D]">
                {item}
              </div>
            ))}
          </div>
        ...
        <section className="mt-8 grid gap-4 md:grid-cols-3">
          {["Швидкий виїзд кур'єра", "Зрозумілі очікування за часом", "Підготовлено до інтеграції з мапою"].map((item) => (
            <div key={item} className="card-white rounded-[26px] p-5 text-center text-lg font-bold text-[#171717]">
              {item}
            </div>
          ))}
        </section>
